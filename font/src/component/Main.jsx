...
const Main = () => {
  ...
  const submit = async () => {
    ...
    console.log(import.meta.env.VITE_ALL_FEATCH_ALL_TIME);
    console.log(import.meta.env.VITE_COUSTOMIZED_PLAYLIST_URl);
  ...
  const submitSelection = async () => {
    ...
    try {
      ...
      setTotalTime(response.data);
      showToast(
        "Selected video configuration submitted successfully!",
        "success",
      );
    } catch (error) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-10 min-h-screen text-white font-sans">
            ...
            <div className="bg-green-950/20 border border-green-500/30 rounded-xl p-4 text-center flex flex-col justify-between min-h-[140px] shadow-md shadow-green-500/5">
              ...
              <div className="flex items-center justify-center my-1">
                <div className="relative flex items-center bg-black/60 border border-green-500/30 focus-within:border-green-400 focus-within:ring-1 focus-within:ring-green-400/30 rounded-lg px-2 py-0.5 transition-all">
                  ...
                  />
                  <span className="text-xs text-green-400 font-bold ml-0.5 select-none">
                    x
                  </span>
                </div>
              </div>
              <div className="flex items-center justify-center min-h-[44px]">
                <span className="text-white text-sm font-semibold leading-relaxed">
                  {customSpeed &&
                  !isNaN(parseFloat(customSpeed)) &&
                  parseFloat(customSpeed) > 0 ? (
            ...
            <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
              {videos.map((video, index) => {
                ...
                return (
                  <div
                    key={video.id}
                    className={`rounded-xl overflow-hidden p-4 border transition-all duration-300 ${
                      isChecked
      ...
      {toast && (
        <div
          className={`fixed top-5 right-5 z-50 flex items-center gap-3 border backdrop-blur-md rounded-xl p-4 shadow-2xl max-w-sm transition-all duration-300 transform translate-y-0 scale-100 ${
            toast.type === "success"
